		...
		try {
			if (client.c_singleFrames.containsKey(infos[0])) {
				if (infos.length>4){
					...
					String s = JOptionPane.showInputDialog("对方希望发文件给你，是否接收[是/否]（仅限此两个字符）");
					if (s.equals("是")){
						String filepath=infos[4];
						JFileChooser chooser=new JFileChooser("请选择保存的目录");
						...
						if (i==JFileChooser.APPROVE_OPTION){
							...
							File newFile = new File(targetPath + File.separator + oleFile.getName());
							FileInputStream fileInputStream = new FileInputStream(oleFile);
				...
				if (infos.length>4){
					String s = JOptionPane.showInputDialog("对方希望发文件给你，是否接收");
					if (s.equals("是")){
						String filepath=infos[4];
						JFileChooser chooser=new JFileChooser("请选择保存的目录");
						chooser.setMultiSelectionEnabled(false);
